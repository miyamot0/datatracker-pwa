...
To duplicate an **Evaluation** folder, you can click the "Duplicate" button associated with the specific folder you wish to copy. This will create a new **Evaluation** folder with the same name appended with "Copy" (e.g., "Functional Analysis Copy") unless you elect to name this otherwise. The duplicated folder will contain all the same data files, notes, and conditions as the original folder, allowing you to easily create a new evaluation based on an existing one. This is particularly useful when building from a prior experimental assessment (e.g., functional analysis conditions serving as a baseline for treatment development).

By default, the permission level does not support copying/renaming folders. To enable this, you will need to adjust the permission level for the user role in question. This can be done by navigating to the "Settings" section of the program, selecting "Administrative," and then allowing "Elevated Privileges", which unlocks copying/editing/deleting functionality. As a general rule, we recommend leaving this off to prevent accidental data loss; however, if you have a need to duplicate or rename folders this can be briefly enabled then disabled.
...
### Renaming Evaluation Folders

In addition to duplicating **Evaluation** folders, you can also rename them to better reflect their purpose or the specific context they represent. This is potentially useful in cases when the goals of the evaluation have shift or need to be amended to support clarity/organization. For example, if you initially created an **Evaluation** folder labeled "Demand Treatment Evaluation" but later incorporated an addition function, you could adjust the name to better reflect the new focus.
