...
import { ArrowDown, ArrowUp, ChevronDown, DeleteIcon, EditIcon } from 'lucide-react';
...
export default function KeySetEditor({
  ...
  const [editingDerivedKey, setEditingDerivedKey] = useState<LogicState | null>(null);
  ...
  const addDerivedKeyCallback = async (logic: LogicState) => {
    ...
    if (editingDerivedKey) {
      ...
      newDerivedKeys = KeySetObject.DerivedKeys.map((k) => (k.id === logic.id ? logic : k));
    } else {
      // Add mode
      newDerivedKeys = [...KeySetObject.DerivedKeys, logic];
    }
    const new_state = {
      ...KeySetObject,
                    ...
                    <TableRow key={index} className="bg-muted">
                      ...
                      <TableCell className="flex flex-row gap-2 justify-end">
                        <Button
                          ...
                          className="shadow"
                          title="Edit"
                          onClick={() => setEditingDerivedKey(state)}
                        >
                          <EditIcon size={14} />
                        </Button>
                        <Button
