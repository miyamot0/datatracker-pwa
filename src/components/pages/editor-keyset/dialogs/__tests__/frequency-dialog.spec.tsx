...
describe('FrequencyDialogKeyCreator', () => {
  ...
  it('renders the Add Key trigger button', async () => {
    await render(
      <FolderContextProvider>
        <FrequencyDialogKeyCreator KeySet={makeKeyset()} Callback={mockCallback} />
      </FolderContextProvider>,
    );
  ...
  it('description input updates on change', async () => {
    ...
    await page.getByText('Add Key').click();
    await page.getByRole('textbox').first().fill('Aggression');
    await expect.element(page.getByRole('textbox').first()).toHaveValue('Aggression');
  });
