...
const makeAssignedKeyset = (): KeySet =>
  ({
    ...makeKeyset(),
    DurationKeys: [{ KeyName: 'a', KeyDescription: 'Existing', KeyCode: 65 }],
  }) as any;

const renderDialog = async (keyset: KeySet = makeKeyset()) => {
  await render(
    <FolderContextProvider>
      <DurationDialogKeyCreator KeySet={keyset} Callback={mockCallback} />
    </FolderContextProvider>,
  );
};
...
describe('DurationDialogKeyCreator', () => {
  ...
  it('description input updates on change', async () => {
    await renderDialog();
    await openDialog();
    await descriptionInput().fill('Reinforcement');
    await expect.element(descriptionInput()).toHaveValue('Reinforcement');
  });
  ...
  it('captures an allowed key press into the key capture input', async () => {
    await renderDialog();
    await openDialog();
  ...
  it('submits via Enter key in key capture input', async () => {
    ...
    expect(mockCallback).toHaveBeenCalledWith(
      expect.any(Object),
      expect.objectContaining({ KeyDescription: 'Reinforcement', KeyName: 'a', KeyCode: 65 }),
      'Duration',
    );
  });
