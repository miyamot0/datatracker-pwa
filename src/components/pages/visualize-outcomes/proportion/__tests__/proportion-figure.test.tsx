...
describe('ProportionFigureVisualization', () => {
  ...
  it('renders the BaseChart component', () => {
    render(<ProportionFigureVisualization {...defaultProps} />);
    expect(screen.getByTestId('base-chart')).not.toBeNull();
  });

  it('passes correct title to BaseChart', () => {
    render(<ProportionFigureVisualization {...defaultProps} />);
    expect(screen.getByTestId('chart-title').textContent).toBe('Visualization of Data as Proportion of Session');
  });

  it('passes FigureTextSize to BaseChart', () => {
    render(<ProportionFigureVisualization {...defaultProps} FigureTextSize="large" />);
    expect(screen.getByTestId('chart-text-size').textContent).toBe('large');
  });

  it('passes ConnectSpans=true to BaseChart', () => {
    render(<ProportionFigureVisualization {...defaultProps} ConnectSpans={true} />);
    expect(screen.getByTestId('chart-connect').textContent).toBe('true');
  });

  it('passes ConnectSpans=false to BaseChart', () => {
    render(<ProportionFigureVisualization {...defaultProps} ConnectSpans={false} />);
