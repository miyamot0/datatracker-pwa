...
describe('DiagnosticsPage', () => {
  ...
  describe('Issues Display', () => {
    ...
    it('displays issues list when issues exist', async () => {
      ...
      const issuesList = page.getByRole('list');
      const listItems = issuesList.locate('li').elements();
    ...
    it('does not display issues list when no issues exist', async () => {
      ...
      const issuesList = page.queryByRole('list');
      expect(issuesList.elements()).toHaveLength(0);
    });
  });
  ...
  describe('Recommendations Display', () => {
    ...
    it('does not display recommendations list when no recommendations exist', async () => {
      ...
      // Only issues list should exist (which is empty), no recommendations list
      const lists = page.queryByRole('list').elements();
  ...
  describe('AdaptiveBadge Component Behavior', () => {
    it('applies correct classes for enabled state', async () => {
      ...
      enabledBadges.forEach(badge => {
        expect(badge).toHaveClass('bg-green-500', 'text-white', 'hover:bg-green-500', 'cursor-default', 'select-none', 'whitespace-nowrap');
      });
    });
    ...
    it('applies correct classes for disabled state', async () => {
      ...
      disabledBadges.forEach(badge => {
        expect(badge).toHaveClass('bg-red-500', 'hover:bg-red-500', 'text-white', 'cursor-default', 'select-none', 'whitespace-nowrap');
      });
    });
  });
  ...
  describe('Integration Tests', () => {
    it('displays complete diagnostic information with mixed states', async () => {
      ...
      await renderDiagnosticsPage(
        { queries: { staleTime: 30000, gcTime: 180000 } },
        complexState
      );
      ...
      // Check issues
      expect(page.getByText('1')).toBeInTheDocument(); // Issues count
      expect(page.getByText('SharedArrayBuffer not available')).toBeInTheDocument();
      
      // Check recommendations  
      expect(page.getByText('2')).toBeInTheDocument(); // Recommendations count
